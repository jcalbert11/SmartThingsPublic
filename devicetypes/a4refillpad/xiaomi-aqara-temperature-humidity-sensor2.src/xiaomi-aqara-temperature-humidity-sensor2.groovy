 ...
 */
metadata {
	definition (name: "Xiaomi Aqara Temperature Humidity Sensor2", namespace: "a4refillpad", author: "a4refillpad") {
	...
	preferences {
        ...
        section {
            ...
            input "humidOffset", "number", title: "%", description: "Adjust humidity by this %", range: "*..*", displayDuringSetup: false
		}
    }
	...
	tiles(scale: 2) {
        ...
        standardTile("pressure", "device.pressure", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
			state "default", label:'${currentValue} inHG', icon:"st.Weather.weather1"
		}
...
def parse(String description) {
	...
	def unit = (name == "temperature") ? getTemperatureScale() : ((name == "humidity") ? "%" : ((name == "pressure")? "inHG": null))
...
private String parseReadAttrMessage(String description) {
    ...
    if (cluster == "0403" && attrId == "0000") {
         ...
         if (pressOffset)
         {
           result = ((pressureval + pressOffset)/100 * 0.295301 as Float)
         }
         else
         {
           result = (pressureval/100 * 0.295301 as Float)
         }
    } 
