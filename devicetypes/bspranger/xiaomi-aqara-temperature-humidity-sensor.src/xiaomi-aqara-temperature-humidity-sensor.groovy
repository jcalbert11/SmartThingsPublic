...
metadata {
    ...
    preferences {
        ...
        //    input "tempOffset", "number", title:"Degrees", description:"Adjust temperature by this many degrees", range:"*..*", defaultValue: 0, displayDuringSetup: true, required: true
        //}
        //section {
        ...
        //    input "humidOffset", "number", title:"Humidity", description:"Adjust humidity by this many units", range: "*..*", defaultValue: 0, displayDuringSetup: true, required: true
        //}
        //section {
        ...
        //    input "pressOffset", "number", title:"Pressure", description:"Adjust pressure by this many units", range: "*..*", defaultValue: 0, displayDuringSetup: true, required: true
        //}
        section {
            input title:"Sensor Offsets", description:"This feature allows you to correct any variations by selecting an offset. Ex: If your sensor consistently reports a temp that's 5 degrees too warm, you'd enter '-5'. If 3 degrees too cold, enter '+3'. Please note, any changes will take effect only on the NEXT temperature change.", displayDuringSetup: false, type:"paragraph", element:"paragraph"
            input "tempOffset", "number", title:"Degrees", description:"Adjust temperature by this many degrees", range:"*..*", defaultValue: 0, displayDuringSetup: false, required: false
            input "humidOffset", "number", title:"Humidity", description:"Adjust humidity by this many units", range: "*..*", defaultValue: 0, displayDuringSetup: false, required: false
        	input "pressOffset", "number", title:"Pressure", description:"Adjust pressure by this many units", range: "*..*", defaultValue: 0, displayDuringSetup: false, required: false
        }
        section {
			input title: "Localization Settings", description: "Only change the settings below if you know what you're doing", displayDuringSetup: false, type: "paragraph", element: "paragraph"
            input name:"PressureUnits", type:"enum", title:"Pressure Units", options:["mbar", "kPa", "inHg", "mmHg"], description:"Sets the unit in which pressure will be reported", defaultValue:"inHG", displayDuringSetup: false, required: true
			input name: "dateformat", type: "enum", title: "Set Date Format\n US (MDY) - UK (DMY) - Other (YMD)", description: "Date Format", defaultValue: "US", displayDuringSetup: false, required: false, options:["US","UK","Other"]
        }
		section {    
			...
			input name: "voltsmax", title: "Max Volts\nA battery is at 100% at __ volts\nRange 2.8 to 3.4", type: "decimal", range: "2.8..3.4", defaultValue: 3, displayDuringSetup: false, required: false
			input name: "voltsmin", title: "Min Volts\nA battery is at 0% (needs replacing) at __ volts\nRange 2.0 to 2.7", type: "decimal", range: "2..2.7", defaultValue: 2.5, displayDuringSetup: false, required: false
    	}
	}
...
private Map parseHumidity(String description){
    ...
    if (pct.isNumber()) {
        if (settings.tempOffset) {
        	pct =  Math.round(new BigDecimal(pct) + humidOffset)
        } else {
...
private Map getBatteryResult(rawValue) {
	...
	maxVolts = voltsmax
    log.debug "${device.displayName}: Raw Volts: ${rawVolts}"
